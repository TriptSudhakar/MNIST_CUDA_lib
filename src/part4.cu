...
int main(int argc, char** argv) {
    ...
    else
    {
        ...
        cudaMalloc(&conv1Output, streamCount * 20 * 24 * 24 * sizeof(float));
        cudaMalloc(&pool1Output, streamCount * 20 * 12 * 12 * sizeof(float));
        cudaMalloc(&conv2Output, streamCount * 50 * 8 * 8 * sizeof(float));
        cudaMalloc(&pool2Output, streamCount * 50 * 4 * 4 * sizeof(float));
        cudaMalloc(&fc1Output, streamCount * 500 * sizeof(float));
        cudaMalloc(&fc2Output, streamCount * 10 * sizeof(float));
        ...
        for(int i=0;i<fileCount;++i) {
            convolution<<<gridDim1, blockDim1, 0, streams[(i % streamCount)]>>>(input + i * inputRows * inputCols, conv1Weights, conv1Biases, conv1Output + (i % streamCount) * 20 * 24 * 24, 28, 28, 1, 5, 5, 24, 24, 20);
            maxPooling<<<gridDim2, blockDim2, 0, streams[(i % streamCount)]>>>(conv1Output + (i % streamCount) * 20 * 24 * 24, pool1Output + (i % streamCount) * 20 * 12 * 12, 24, 24, 2, 12, 12, 20);
            
            convolution<<<gridDim3, blockDim3, 0, streams[(i % streamCount)]>>>(pool1Output + (i % streamCount) * 20 * 12 * 12, conv2Weights, conv2Biases, conv2Output + (i % streamCount) * 50 * 8 * 8, 12, 12, 20, 5, 5, 8, 8, 50);
            maxPooling<<<gridDim4, blockDim4, 0, streams[(i % streamCount)]>>>(conv2Output + (i % streamCount) * 50 * 8 * 8, pool2Output + (i % streamCount) * 50 * 4 * 4, 8, 8, 2, 4, 4, 50);
            
            convolution<<<gridDim5, blockDim5, 0, streams[(i % streamCount)]>>>(pool2Output + (i % streamCount) * 50 * 4 * 4, fc1Weights, fc1Biases, fc1Output + (i % streamCount) * 500, 4, 4, 50, 4, 4, 1, 1, 500);
            relu<<<256, 2, 0, streams[(i % streamCount)]>>>(fc1Output + (i % streamCount) * 500, 500);
            
            convolution<<<gridDim6, blockDim6, 0, streams[(i % streamCount)]>>>(fc1Output + (i % streamCount) * 500, fc2Weights, fc2Biases, fc2Output + (i % streamCount) * 10, 1, 1, 500, 1, 1, 1, 1, 10);
            softmax<<<32, 1, 0, streams[(i % streamCount)]>>>(fc2Output + (i % streamCount) * 10, output + i * 10, 10);
        }
